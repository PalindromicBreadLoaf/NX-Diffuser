    ...
    for key, val in data.items():
        ...
        if val.get("offset") is not None:
            ...
            fills.append((
                ...
                val.get("type", ""),
                sym,
                synthetic_to_physical(synthetic_offset),
                size,
                n64_address,
            ))
...
with open(BINDING_C, "w") as f:
    ...
    f.write("\nenum { GDX_EK_ASSET_OTHER, GDX_EK_ASSET_GFX, GDX_EK_ASSET_VP };\n")
    ...
    f.write(" * address ranges. Gfx commands remain big-endian/raw; Vp fields are\n")
    f.write(" * converted to native s16 because the host interpreter reads Vp directly. */\n")
