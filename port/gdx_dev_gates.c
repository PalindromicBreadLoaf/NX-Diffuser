...
static const GdxGateDesc kGates[GDX_GATE_COUNT] = {
    ...
    /* DIAG_IMGUI        */ { "GDX_DIAG_IMGUI",           "gDevTools.Diag.ImGuiMenu",         "Menu selection + draw data",    "Once a second while the settings menu is open, reports the resolved selection, the colour and rectangle each highlight is drawn with, and how much geometry the frame produced. For diagnosing a highlight that is drawn but not visible.", GDX_GATE_ENV_PRESENCE,    GDX_GATE_GROUP_GRAPHICS,   GDX_GATE_BUCKET_DIAG,     0 },
