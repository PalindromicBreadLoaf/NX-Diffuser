...
#ifdef _WIN32
    n = snprintf(outPath, outCap, "%s\\ghost_%08X.gdg", directory, (uint32_t) encodedCourseIndex);
#else
    n = snprintf(outPath, outCap, "%s/ghost_%08X.gdg", directory, (uint32_t) encodedCourseIndex);
#endif
...
#ifdef _WIN32
    n = snprintf(outPath, outCap, "%s\\ghost_%08X_%016llX.gdg", directory, (uint32_t) encodedCourseIndex,
...
static int gdx_ghost_selection_write(const GdxGhostSelection* selections, int count) {
    ...
    for (i = 0; i < count && rc == GDX_GHOST_OK; i++) {
        if (fprintf(f, "%08X %016llX\n", (uint32_t) selections[i].encodedCourseIndex,
